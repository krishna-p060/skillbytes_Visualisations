...
function drawMode() {
    if (points.length > 0) {
        mode = calculateMode(points);
        let modeX = map(mode, minRange, maxRange, boundaryStart, boundaryEnd);
        fill(255, 165, 0);
        ellipse(modeX, height / 2 - 20, 10, 10);
        fill(0);
        text(`Mode: ${mode.toFixed(2)}`, modeX, height / 2 - 80);
    }
}
